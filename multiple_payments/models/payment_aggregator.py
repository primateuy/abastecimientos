...
class PaymentAggregator(models.Model):
    ...
    def _create_lines_payment_payments(self):
        ...
        for payment_method in self.mps_payment_methods_line_ids:
            ...
            payment_details['invoice_date'] = payment_date
    ...
    def button_reconciliate_payments(self):
        ...
        # Iteramos sobre facturas a conciliar
        for credit_line in self.account_move_line_payment_agg_ids:
            invoice_lines = credit_line.move_id.line_ids.filtered(
                lambda line: line.account_id.account_type in ['asset_receivable', 'liability_payable'] and not line.reconciled
            )

            amount_to_reconcile = credit_line.payment_aggregator_total_import

            # Asegurar que todas las líneas tengan fecha válida
            valid_date = self.date or fields.Date.today()
            for line in invoice_lines:
                if not line.date or line.date is False:
                    line.write({'date': valid_date})

            # _logger.info("amount_to_reconcile")
            # _logger.info(amount_to_reconcile)

            # Recorremos pagos hasta cubrir el importe de la factura
            for payment in payments:
                ...
                for payment_line in payment_lines:
                    ...
                    available_payment = abs(payment_line.amount_residual)
                    # _logger.info("available_payment")
                    # _logger.info(available_payment)
                    if available_payment <= 0:
                        continue
                        

                    # Determinar el importe a conciliar
                    reconcile_amount = min(amount_to_reconcile, available_payment)

                    # _logger.info("reconcile_amount")
                    # _logger.info(reconcile_amount)

                    # Evitar duplicados
                    partial_reconcile = account_partial_reconcile.search([
                        ("debit_move_id", "=", invoice_lines.id),
                        ("credit_move_id", "=", payment_line.id)
                    ], limit=1)

                    # _logger.info("partial_reconcile")
                    # _logger.info(partial_reconcile)

                    if not partial_reconcile and reconcile_amount > 0:
                        account_partial_reconcile.create({
                            'debit_move_id': invoice_lines.id,
                            'credit_move_id': payment_line.id,
                            'amount': reconcile_amount,  # en moneda de la compañía
                            'debit_amount_currency': reconcile_amount,
                            'credit_amount_currency': reconcile_amount,
                            'max_date': self.date
                        })

                    # Reducimos el saldo pendiente de la factura
                    amount_to_reconcile -= reconcile_amount